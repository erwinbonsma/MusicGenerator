...
class MusicData:
    def __init__(self, filename):
        ...
        self.sfx_ids = set()
        if args.all_sfxs:
            self.sfx_ids.update(range(len(self.sfxs)))
        else:
        ...
        for song in self.songs:
            for pattern_id in song.pattern_ids:
                ...
                for sfx_id in pattern.sfx_ids:
                    self.sfx_ids.add(sfx_id)

    def check(self):
        for sfx_id in self.sfx_ids:
        ...
        max_volume = max([sfx.max_volume() for sfx in [self.sfxs[sfx_id] for sfx_id in self.sfx_ids]])
        if max_volume < 7:
